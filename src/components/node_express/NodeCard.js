    ...
    return (
        <div className='card_block'>
          ...
          <Card border="primary" className="cardContainer" style={{ width: '35rem' }}>
            <Card.Img variant="top" className='cardPhoto' src={`http://localhost:3004${article.featured_image}`} />
            <Card.Body className='card_body'>
                <Card.Title className='card_title'>{article.name}</Card.Title>
                {id ? <>{article.description}</> : (<Card.Text><Link to={`/${article.id}`}>Read article</Link></Card.Text>)}
